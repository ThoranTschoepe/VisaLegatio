...
def create_tables():
    ...
    # Lightweight compatibility migrations for legacy SQLite databases
    with engine.connect() as connection:
        result = connection.execute(text("PRAGMA table_info(flagged_documents)"))
        column_names = {row[1] for row in result}

        alterations: list[str] = []
        if "flag_category_id" not in column_names:
            alterations.append("ALTER TABLE flagged_documents ADD COLUMN flag_category_id INTEGER")
        if "bias_review_id" not in column_names:
            alterations.append("ALTER TABLE flagged_documents ADD COLUMN bias_review_id VARCHAR")
