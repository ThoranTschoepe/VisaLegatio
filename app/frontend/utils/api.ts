...
class APIClient {
  ...
  async getApplicationDocuments(applicationId: string): Promise<Array<{
    ...
    try {
      const response = await this.request<any[]>(`/documents/list/${applicationId}`)
      ...
      return response.map(doc => ({
        ...
        view_url: doc.view_url ?? doc.viewUrl ?? null,
        download_url: doc.download_url ?? doc.downloadUrl ?? null,
        file_exists: doc.file_exists ?? doc.fileExists ?? Boolean(doc.view_url || doc.download_url || doc.file_path),
      }))
    } catch (primaryError) {
      console.warn('Falling back to legacy document endpoint:', primaryError)

      const fallback = await this.request<any[]>(`/documents/application/${applicationId}`)
      ...
      return fallback.map(doc => ({
        ...
        uploaded_at: doc.uploaded_at,
        file_path: doc.file_path,
        view_url: doc.file_path ? `/api/documents/view/${applicationId}/${doc.name}` : null,
        download_url: doc.file_path ? `/api/documents/download/${applicationId}/${doc.name}` : null,
        file_exists: Boolean(doc.file_path),
      }))
    }
  }
  ...
  private transformFlaggedDocument(payload: any): FlaggedDocument {
    return {
      ...
      requiresFollowUp: payload.audit_decision_requires_follow_up ?? undefined,
      applicantVisible:
        payload.applicant_visible ??
        (payload.audit_decision_requires_follow_up !== undefined && payload.audit_decision_requires_follow_up !== null
          ? Boolean(payload.audit_decision_requires_follow_up)
          : undefined),
    }
  }
