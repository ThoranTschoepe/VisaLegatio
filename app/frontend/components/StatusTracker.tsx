...
import { VisaApplication, ApplicationStatus, VisaType, FlaggedDocument } from '@/types'
...
export default function StatusTracker({ applicationId, onNewApplication, onNavigateToDocuments }: StatusTrackerProps) {
  ...
  const loadApplicationStatus = async () => {
    try {
      ...
      const applicantFlags = (data.flaggedDocuments || []).filter(flag => {
        if (flag.applicantVisible !== undefined) {
          return flag.applicantVisible
        }
        if (flag.requiresFollowUp !== undefined) {
          return Boolean(flag.requiresFollowUp)
        }
        return false
      })
      setFlaggedDocuments(applicantFlags)
