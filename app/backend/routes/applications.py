...
FLAG_AUDITED_STATUS = "flag_audited"
...
def determine_application_status(application_id: str, visa_type: str, db: Session) -> str:
    ...
    active_flag_query = db.query(FlaggedDocument).filter(
        FlaggedDocument.application_id == application_id,
        FlaggedDocument.resolved == False
    )
    active_flag_count = active_flag_query.count()
    if active_flag_count > 0:
        ...
        if latest_review and latest_review.audit_status and latest_review.audit_status != "pending":
            return FLAG_AUDITED_STATUS
...
DecisionLabelLookup = Dict[str, str]
FollowUpLookup = Dict[Tuple[str, str], bool]


def _build_decision_metadata(catalog: Dict[str, Any]) -> Tuple[DecisionLabelLookup, FollowUpLookup]:
    decisions = catalog.get("decisions", []) if catalog else []
    matrix = catalog.get("matrix", {}) if catalog else {}

    decision_lookup: DecisionLabelLookup = {
        decision.get("code"): decision.get("label")
        for decision in decisions
        if decision.get("code")
    }

    follow_up_lookup: FollowUpLookup = {}
    for flag_code, entries in matrix.items():
        for entry in entries or []:
            code = entry.get("code")
            if not code:
                continue
            follow_up_lookup[(flag_code, code)] = bool(entry.get("requiresFollowUp"))

    return decision_lookup, follow_up_lookup
...
def _serialize_flagged_document(
    ...
    flag_type = flag.flag_type or (flag.category.code if flag.category else None)
    requires_follow_up = None
    if flag_type and audit_code:
        requires_follow_up = follow_up_lookup.get((flag_type, audit_code))
    applicant_visible = bool(requires_follow_up)
    ...
    return FlaggedDocumentResponse(
        ...
        audit_decision_requires_follow_up=requires_follow_up,
...
async def get_applications(
    ...
    decision_lookup, follow_up_lookup = _build_decision_metadata(catalog)
    ...
    for app in applications:
        ...
        flagged_documents = [
            _serialize_flagged_document(flag, decision_lookup, follow_up_lookup)
